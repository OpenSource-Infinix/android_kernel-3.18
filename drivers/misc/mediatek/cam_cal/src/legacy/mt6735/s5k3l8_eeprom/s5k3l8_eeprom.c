...
#define CAM_CALDB(format, args...)    printk(KERN_ERR format, ##args)
#else
#define CAM_CALINF(x, ...)
#define CAM_CALDB(x, ...)
#define CAM_CALERR(x, ...)
...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "cam_cal_drv_s5k3l8"
...
{ .compatible = "mediatek,cam_cal_drv_s5k3l8", },
};
static int s5k3l8_selective_read_region(u32 addr, u8 *data, u16 i2c_id, u32 size);
...
static bool selective_read_byte(unsigned short addr,unsigned char *data,u16 i2c_id )
{
...
static int s5k3l8_selective_read_region(u32 addr, BYTE *data, u16 i2c_id, u32 size)
{
	...
	int ret = 0;
	CAM_CALDB("s5k3l8_selective_read_region size_to_read =%d \n", size_to_read);
	while (size_to_read > 0) {
		if (selective_read_byte(addr, (u8 *)buff, i2c_id)) {
			...
			CAM_CALERR("err: size_to_read =%d \n", size_to_read);
			break;
		}
	}
	//CAM_CALDB("s5k3l8_selective_read_region addr =%x size %d data read = %d\n", addr, size, ret);
	return ret;
}
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		do_gettimeofday(&ktv1);
#endif
		CAM_CALDB("[CAM_CAL] offset 0x%x,length=%d\n", ptempbuf->u4Offset, ptempbuf->u4Length);
	
		/**pu1Params = 0;*/
		i4RetValue = s5k3l8_selective_read_region(ptempbuf->u4Offset, pu1Params, S5K3L8_OTP_DEVICE_ID, ptempbuf->u4Length);
	...
	kfree(pBuff);
	kfree(pu1Params);
	CAM_CALDB("[CAM_CAL] kernel return value =%d\n",i4RetValue);
	return i4RetValue;
}
...
static int CAM_CAL_i2c_probe_S5k3l8(struct i2c_client *client, const struct i2c_device_id *id)
{
	int i4RetValue = 0;
	CAM_CALDB("[S5k3l8_CAL] Attach I2C\n");
    ...
    g_pstI2Cclient->addr = S5K3L8_OTP_DEVICE_ID;
	...
	if (i4RetValue) {
		CAM_CALDB("[S5k3l8_CAL] register char device failed!\n");
		return i4RetValue;
	}
	CAM_CALDB("[S5k3l8_CAL] Attached!!\n");
	return 0;
}

static int CAM_CAL_i2c_remove_S5k3l8(struct i2c_client *client)
{
	return 0;
}

static struct i2c_driver CAM_CAL_i2c_driver_S5k3l8 = {
	.probe = CAM_CAL_i2c_probe_S5k3l8,
	.remove = CAM_CAL_i2c_remove_S5k3l8,
...
{
	CAM_CALDB("[S5k3l8_CAL] CAM_CAL_platform_probe !!\n");
	return i2c_add_driver(&CAM_CAL_i2c_driver_S5k3l8);
}

static int CAM_CAL_remove(struct platform_device *pdev)
{
	i2c_del_driver(&CAM_CAL_i2c_driver_S5k3l8);
	return 0;
}
...
{
	CAM_CALDB("[CAM_CAL] liyangadd_otp:CAM_CAL_i2C_init Start!\n");
...
MODULE_AUTHOR("yang.li3 <yang.li3@reallytek.com>");
MODULE_LICENSE("GPL");
