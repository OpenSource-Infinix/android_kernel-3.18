...
static unsigned int lcm_compare_cpt_id(void)
{
	int data[4] = {0,0,0,0};
	int res = 0;
	int rawdata = 0;
	...
	if(res < 0)
	{
		LCM_ERROR("(%s) HX8394F_CPT_fhd_dsi_vdo_tcl  get lcm chip id vol fail\n", __func__);
		return 0;
	}
#endif
	lcm_vol = data[0]*1000+data[1]*10;
	LCM_DEBUG("(%s) HX8394F_CPT_fhd_dsi_vdo_tcl lcm chip id adc raw data:%d, lcm_vol:%d\n", __func__, rawdata, lcm_vol);
